...
celldata = load([datadir 'cell_type_genes_indices']);
selcells = false(size(expressionlevels.MicroarrayExpression,1),1);
selcells(celldata.geneSubSets.(celltypes{cnr})) = true;
allgindices = 1:size(expressionlevels.MicroarrayExpression,1);
selgenes = allgindices(indexdata.probe2gene_index>0 & selcells);
...
%% Save mapped data
save(['MappedGenesSelection2' celltypes{cnr} id],'X','mappedX','selgenes','coords',...
