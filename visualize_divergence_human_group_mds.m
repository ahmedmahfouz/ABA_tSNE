...
%% Data
resultsdatadir = [LUMCDATADIR 'tSNE_ABA/normalized/results_human_average_group_tsne_pca_mds/'];
ids = {'9861','10021','12876','14380','15496','15697'};
datadir = [LUMCDATADIR 'tSNE_ABA/rawData_25Feb2014/normalized_microarray_donor'];
% Load mapped data
load([resultsdatadir 'MDSHumanGenesGroupAverageMeanSub'],'mappedX2');
% Load voxel labels
mainStructures = {'FL', 'PL', 'TL', 'OL', 'HiF', 'Str', 'GP', 'Amg', 'TH', ...
    'Hy', 'MES', 'Pons', 'MY', 'Cb', 'WM'};

%% Get grouped human data
nids = length(ids);

voxelLabels = [];
for inr = 1:nids
    load([datadir ids{inr} '/sampleLables']);
    voxelLabels = [voxelLabels; sampleLables(:)];
end
nlabels = max(voxelLabels);

%% Determine histogram size
mappedX = mappedX2(:,1:2);
minbin = min(mappedX);
maxbin = max(mappedX);
nbins = 40;
