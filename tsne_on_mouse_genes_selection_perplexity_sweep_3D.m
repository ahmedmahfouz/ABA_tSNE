...
%     mappedRGB = applycform([50*ones(size(mappedab,1),1) mappedab],tC);
    mappedRGB = (mappedX-min(mappedX(:)))/(max(mappedX(:))-min(mappedX(:)));
    
    %% Save mapped data
    save([outdir sprintf('MappedMouseGenesPerplexitySweep3Dperp%d',perplexity)],'X','mappedX','coords',...
    ...
    for snr = 1:nsamples
        ...
        plot3(mappedX(snr,1),mappedX(snr,2),mappedX(snr,3),'.','Color',colorRGB);
    end
    view(3)
    axis equal
    
    saveas(3,[outdir sprintf('MappedMousePerplexitySweep3Dperp%d',perplexity)]);
    saveas(3,[outdir sprintf('MappedMousePerplexitySweep3Dperp%d.png',perplexity)]);
    ...
    cm = jet(67);
    for snr = 1:nsamples
        plot3(mappedX(snr,1),mappedX(snr,2),mappedX(snr,3),'.','Color',cm(coords(snr,1),:));
    end
    view(3)
    axis equal
    
    saveas(3,[outdir sprintf('MappedMousePerplexitySweepSlice3Dperp%d',perplexity)]);
    saveas(3,[outdir sprintf('MappedMousePerplexitySweepSlice3Dperp%d.png',perplexity)]);
    ...
    save([outdir sprintf('MappedMouseGeneVolumePerplexitySweep3Dperp%d',perplexity)],'imgr','imgg','imgb');
    ...
    save([outdir 'MappedMouseGeneVolumePerplexitySweepRegions3D'],'imgr','imgg','imgb');
end
