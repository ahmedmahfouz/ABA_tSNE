    ...
    if sum(mapsel)>0
        ...
        if size(pts,1) > 3
            for dnr = 1:ndims
                ...
                F = scatteredInterpolant(pts,ev,'nearest','nearest');
                labvals(:,dnr) = F(labicoords);
...
colorimgr = ones(size(atlas.img));
colorimgg = ones(size(atlas.img));
colorimgb = ones(size(atlas.img));
colorimgr(labind) = mappedRGB(:,1);
colorimgg(labind) = mappedRGB(:,2);
colorimgb(labind) = mappedRGB(:,3);
lcolorimg = joinchannels('rgb',colorimgr,colorimgg,colorimgb);

% Select images per slice
ltimg = cat(3,squeeze(colorimgr(:,:,tslice)),squeeze(colorimgg(:,:,tslice)),squeeze(colorimgb(:,:,tslice)));
lcimg = cat(3,squeeze(colorimgr(:,cslice,:)),squeeze(colorimgg(:,cslice,:)),squeeze(colorimgb(:,cslice,:)));
lsimg = cat(3,squeeze(colorimgr(sslice,:,:)),squeeze(colorimgg(sslice,:,:)),squeeze(colorimgb(sslice,:,:)));
lcimg = flip(permute(lcimg,[2 1 3]),1);
lsimg = flip(permute(lsimg,[2 1 3]),1);
...
set(gcf,'Units','pixel','Position',[50 50 size(ltimg,2)*4 size(ltimg,1)*4],'Color','White');
saveas(gcf,['figure6tsnetransversal' id '_' atlasname '.png']);
...
axis equal off
set(gca,'Units','normalize','Position',[0 0 1 1]);
set(gcf,'Units','pixel','Position',[50 50 size(lsimg,2)*4 size(lsimg,1)*4],'Color','White');
saveas(gcf,['figure6tsnesagittal' id '_' atlasname '.png']);
...
axis equal off
set(gca,'Units','normalize','Position',[0 0 1 1]);
set(gcf,'Units','pixel','Position',[50 50 size(lcimg,2)*4 size(lcimg,1)*4],'Color','White');
saveas(gcf,['figure6tsnecoronal' id '_' atlasname '.png']);
...
rimg = cell(1,3);
for dnr = 1:3
    rimg{dnr} = ones(size(atlas.img));
end
labvals = zeros(size(labicoords,1),3);
for dnr = 1:3
...
% Select images per slice
rtimg = cat(3,squeeze(rimg{1}(:,:,tslice)),squeeze(rimg{2}(:,:,tslice)),squeeze(rimg{3}(:,:,tslice)));
rcimg = cat(3,squeeze(rimg{1}(:,cslice,:)),squeeze(rimg{2}(:,cslice,:)),squeeze(rimg{3}(:,cslice,:)));
rsimg = cat(3,squeeze(rimg{1}(sslice,:,:)),squeeze(rimg{2}(sslice,:,:)),squeeze(rimg{3}(sslice,:,:)));
rcimg = flip(permute(rcimg,[2 1 3]),1);
rsimg = flip(permute(rsimg,[2 1 3]),1);
...
axis equal off
set(gca,'Units','normalize','Position',[0 0 1 1]);
set(gcf,'Units','pixel','Position',[50 50 size(rtimg,2)*4 size(rtimg,1)*4],'Color','White');
saveas(gcf,['figure6regiontransversal' id '_' atlasname '.png']);
...
axis equal off
set(gca,'Units','normalize','Position',[0 0 1 1]);
set(gcf,'Units','pixel','Position',[50 50 size(rsimg,2)*4 size(rsimg,1)*4],'Color','White');
saveas(gcf,['figure6regionsagittal' id '_' atlasname '.png']);
...
axis equal off
set(gca,'Units','normalize','Position',[0 0 1 1]);
set(gcf,'Units','pixel','Position',[50 50 size(rcimg,2)*4 size(rcimg,1)*4],'Color','White');
saveas(gcf,['figure6regioncoronal' id '_' atlasname '.png']);
%%
break
