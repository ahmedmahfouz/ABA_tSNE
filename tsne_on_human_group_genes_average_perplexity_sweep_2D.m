...
initial_dims_vec = [];
perplexity_vec = 2.^(1:10);
% initial_dims_vec = 100;
nperp = length(perplexity_vec);
...
for bnr = 1:nbrains
    ...
    outdir = 'results_human_average_perplexity_sweep/';
...
for pnr = 1:nperp
    ...
    %% Save mapped data
    save([outdir sprintf('MappedHumanGenesAverage2Dperp%d',perplexity)],'X','mappedX','coords',...
    ...
    saveas(2,[outdir sprintf('MappedHumanGenesGroupAverageRegions2Dperp%d',perplexity)]);
    ...
    saveas(3,[outdir sprintf('MappedHumanGenesGroupAverageBrainID2Dperp%d',perplexity)]);
end
